...
class Estrategia:
    def __init__(self, segmento="", cotacao_atual_minima=0, ffo_yield_minimo=0, dividend_yield_minimo=0, p_vp_minimo=0, valor_mercado_minimo=0, liquidez_minima=0, qt_minima_imoveis=0, 
    ...
    def aplica_estrategia(self, fundo: FundoImobiliario):
        ...
        if  (fundo.cotacao_atual < self.cotacao_atual_minima or
                fundo.ffo_yield < self.ffo_yield_minimo or
                fundo.dividend_yield < self.dividend_yield_minimo or
                fundo.p_vp < self.p_vp_minimo or
                fundo.valor_mercado < self.valor_mercado_minimo or
                fundo.liquidez < self.liquidez_minima or
                fundo.qt_imoveis < self.qt_minima_imoveis or
                fundo.preco_m2 < self.valor_minimo_preco_m2 or
                fundo.aluguel_m2 < self.valor_minimo_aluguel_m2 or
                fundo.cap_rate < self.valor_minimo_cap_rate or
                fundo.vacancia_media > self.maxima_vacancia_media):
            return False
        else:
            return True